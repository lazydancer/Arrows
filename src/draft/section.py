...
def flatten(master_data):
    data = copy.deepcopy(master_data) # s is copied
    while any(type(value) is dict for value in data.values()):
        to_remove = [] # keys
        to_add = {} # dictionary
        ...
        for (key, value) in data.items():
            if type(value) is dict:
                # Shift the sub_block
                shift = {(sub_key[0] + key[0], sub_key[1] + key[1]): sub_value for (sub_key, sub_value) in value.items()}
                # remove the key
